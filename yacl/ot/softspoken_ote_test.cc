...
TEST_P(SoftspokenKTest, KWorks) {
  ...
  std::future<void> sender = std::async([&] {
    SoftspokenOtExtSend(lctxs[0], base_ot.recv, std::span(send_out), k, false,
                        mal);
  });
  std::future<void> receiver = std::async([&] {
    SoftspokenOtExtRecv(lctxs[1], base_ot.send, choices, std::span(recv_out), k,
                        false, mal);
  });
  receiver.get();
  sender.get();
...
TEST_P(SoftspokenKTest, ReuseWorks) {
  ...
  std::future<void> sendTask2 =
      std::async([&] { ssSender.Send(lctxs[0], std::span(send_out2), false); });
...
TEST_P(SoftspokenOtExtTest, RotExtWorks) {
  ...
  std::vector<uint128_t> recv_out(num_ot);
  std::future<void> sender = std::async([&] {
    SoftspokenOtExtSend(lctxs[0], base_ot.recv, std::span(send_out), 2, false,
                        mal, compact);
  });
  std::future<void> receiver = std::async([&] {
    SoftspokenOtExtRecv(lctxs[1], base_ot.send, choices, std::span(recv_out), 2,
                        false, mal, compact);
  });
  receiver.get();
  sender.get();
...
TEST_P(SoftspokenOtExtTest, CotExtWorks) {
  ...
  std::future<void> sender = std::async([&] {
    SoftspokenOtExtSend(lctxs[0], base_ot.recv, std::span(send_out), 3, true,
                        mal, compact);
  });
  std::future<void> receiver = std::async([&] {
    SoftspokenOtExtRecv(lctxs[1], base_ot.send, choices, std::span(recv_out), 3,
                        true, mal, compact);
  });
  receiver.get();
  sender.get();
