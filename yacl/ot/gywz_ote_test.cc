...
TEST_P(GywzParamTest, FixIndexSpCotWork) {
  ...
  std::future<void> sender = std::async([&] {
    GywzOtExtRecv_fixed_index(lctxs[0], base_ot.recv, n, std::span(recv_out));
  });
  std::future<void> receiver = std::async([&] {
    GywzOtExtSend_fixed_index(lctxs[1], base_ot.send, n, std::span(send_out));
  });
  sender.get();
  receiver.get();
...
TEST(GywzEdgeTest, Work) {
  ...
  std::future<void> receiver = std::async([&] {
    ASSERT_THROW(GywzOtExtSend(lctxs[1], base_ot.send, n, std::span(send_out)),
                 ::yacl::Exception);
  });
  sender.get();
  receiver.get();
