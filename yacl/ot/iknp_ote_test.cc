...
TEST_P(IknpOtExtTest, Works) {
  ...
  std::future<void> receiver = std::async([&] {
    IknpOtExtRecv(lctxs[1], base_ot.send, choices, std::span(recv_out), false);
  });
  receiver.get();
  sender.get();
...
TEST_P(IknpCotExtTest, Works) {
  ...
  std::future<void> receiver = std::async([&] {
    IknpOtExtRecv(lctxs[1], base_ot.send, choices, std::span(recv_out), true);
  });
  receiver.get();
  sender.get();
...
TEST(IknpOtExtEdgeTest, Test) {
  ...
  }
  {
    ...
    ASSERT_THROW(IknpOtExtSend(lctxs[1], base_ot.recv, std::span(send_out)),
                 ::yacl::Exception);
  }
}
