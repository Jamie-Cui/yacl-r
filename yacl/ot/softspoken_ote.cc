...
void SoftspokenOtExtSender::OneTimeSetup(
  ...
  for (uint64_t i = 0; i < pprf_num_; ++i) {
    ...
    auto c_leaves = std::span(compress_leaves_.data() + i * (pprf_range_ - 1),
                              range_limit - 1);
...
void SoftspokenOtExtReceiver::OneTimeSetup(
  ...
  for (uint64_t i = 0; i < pprf_num_; ++i) {
    ...
    auto sub_ot = dup_base_ot.NextSlice(k_limit);
    // leaves in i-th pprf
    auto leaves = std::span(all_leaves_.data() + i * pprf_range_, range_limit);
...
void SoftspokenOtExtReceiver::GenSfVole(const uint128_t choice,
  ...
  ++counter_;
  // 2. perform CrHash
  ParaCrHashInplace_128(std::span(xor_buff).subspan(0, all_leaves_.size()));
...
void SoftspokenOtExtSender::Send(
  ...
  for (uint64_t t = 0; t < batch_num; ++t) {
    ...
    // 2. smallfield/subspace VOLE
    GenSfVole(std::span(hash_buff), std::span(xor_buff), std::span(recv_U),
              std::span(V[t]));
    if (mal_) {
...
void SoftspokenOtExtReceiver::Recv(const std::shared_ptr<link::Context>& ctx,
  ...
  for (uint64_t t = 0; t < batch_num; ++t) {
    ...
    GenSfVole(choice_ext.data()[super_batch_num * step + t],
              std::span(xor_buff), std::span(U), std::span(W[t]));
    if (mal_) {
  ...
  if (mal_) {
    ...
    CheckMsg<uint128_t> check_msgs;
    auto choice_span = std::span(reinterpret_cast<uint64_t*>(choice_ext.data()),
                                 all_batch_num * 2);
