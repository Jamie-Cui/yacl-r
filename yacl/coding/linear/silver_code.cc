...
#define REGISTER_DUALENCODE(Type)                                              \
  template void SilverCode::DualEncodeInplaceImpl<Type>(std::span<Type> inout) \
      const;                                                                   \
...
template <typename T>
void SilverCode::DualEncodeImpl(std::span<const T> in, std::span<T> out) const {
...
void SilverCode::DualEncode2Impl(std::span<const T> in0, std::span<T> out0,
  ...
  // out = out + buff * L
  LeftEncode2<T, K>(std::span(buff0), out0.subspan(0, n_), std::span(buff1),
                    out1.subspan(0, n_));
}
...
template <typename T, typename K>
void SilverCode::RightEncode2(std::span<T> inout0, std::span<K> inout1) const {
