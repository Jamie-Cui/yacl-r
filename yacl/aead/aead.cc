...
size_t AeadCtx::GetKeySize(AeadType algorithm) {
  switch (algorithm) {
    case AeadType::AES128_GCM:
...
size_t AeadCtx::GetMacSize(AeadType algorithm) {
  switch (algorithm) {
    case AeadType::AES128_GCM:
      return kAes128GcmKeySize;
    case AeadType::AES256_GCM:
...
// (aad).
void AeadCtx::Encrypt(AeadType algorithm, ByteContainerView plaintext,
                      ByteContainerView key, ByteContainerView iv,
                      std::span<uint8_t> ciphertext, std::span<uint8_t> mac,
                      ByteContainerView aad) {
  YACL_ENFORCE(algorithm != AeadType::UNKNOWN);
  switch (algorithm) {
    case AeadType::AES128_GCM: {
...
// additional-authenticated-data (aad).
void AeadCtx::Decrypt(AeadType algorithm, ByteContainerView ciphertext,
