...
KemEncapsulation EcKemEncapsulator::Encapsulate() {
  ...
  OSSL_RET_1(
      EVP_PKEY_CTX_set_kem_op(ctx.get(), OSSL_KEM_PARAM_OPERATION_DHKEM));
...
std::vector<uint8_t> EcKemDecapsulator::Decapsulate(
...
ossl::UniquePkey GenEcKemKeyPair(int curve_nid) {
  EVP_PKEY* pkey = EVP_PKEY_new();  // placeholder

  ossl::UniquePkeyCtx ctx(
      EVP_PKEY_CTX_new_id(EVP_PKEY_EC, /* engine = default */ nullptr));
  YACL_ENFORCE(ctx != nullptr);

  OSSL_RET_1(EVP_PKEY_keygen_init(ctx.get()));
  OSSL_RET_1(EVP_PKEY_CTX_set_ec_paramgen_curve_nid(ctx.get(), curve_nid));

  OSSL_RET_1(EVP_PKEY_keygen(ctx.get(), &pkey));
  return ossl::UniquePkey(pkey);
}

std::pair<Buffer, Buffer> GenEcKemKeyPairToPemBuf(int curve_nid) {
  auto key_pair = GenEcKemKeyPair(curve_nid);
  auto sk_buf = ExportSecretKeyToPemBuf(key_pair);
  auto pk_buf = ExportPublicKeyToPemBuf(key_pair);
