...
#include "yacl/mpc/plaintext_executor.h"
...
TEST(ArithmaticTest, Sub64Test) {
  /* GIVEN */
  std::vector<uint64_t> inputs = {FastRandU64(), FastRandU64()};
  std::vector<uint64_t> result(1);
...
TEST(ArithmaticTest, Neg64Test) {
  /* GIVEN */
  std::vector<uint64_t> inputs = {FastRandU64()};
  std::vector<uint64_t> result(1);
...
TEST(ArithmaticTest, EqzTest) {
  /* GIVEN */
  std::vector<uint64_t> inputs = {FastRandU64() % 2};
  std::vector<uint64_t> result(1);
...
TEST(CryptoTest, Aes128Test) {
  ...
  std::vector<uint128_t> inputs = {FastRandU128(), FastRandU128()};
...
TEST(CryptoTest, Sha256Test) {
  /* GIVEN */
  auto message = FastRandBytes(RandLtN(10));
  ...
  /* THEN */
  auto compare = Sha256Hash().Update(message).CumulativeHash();
