...
#define MP_MIN_DIGIT_COUNT \
  MP_MAX(3, (((int)MP_SIZEOF_BITS(uint64_t) + MP_DIGIT_BIT) - 1) / MP_DIGIT_BIT)
#endif
...
/*
  The mp_log functions rely on the size of mp_word being larger than INT_MAX and
  in case there is a really weird architecture we try to check for it. Not a
  100% reliable test but it has a safe fallback.
 */
...
#define MP_GET_MAG(name, type)                                                \
  type name(const mp_int *a) {                                                \
    int i = MP_MIN(a->used, (int)((MP_SIZEOF_BITS(type) + MP_DIGIT_BIT - 1) / \
                                  MP_DIGIT_BIT));                             \
    type res = 0u;                                                            \
    while (i-- > 0) {                                                         \
